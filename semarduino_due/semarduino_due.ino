...
void loop () {
  ...
  while (g_phase == PHASE_SCANNING) {
    ...
    line = scanAndCopyOneLine(lastLine);
    if (line == lastLine) {
      // hsync has not happened, line counter has not been advanced, keep waiting
      break;
    }
    lastLine = line;
...
int scanAndCopyOneLine(int lastLine) {
  int line = g_numLines;

  // has hsync happened since last time?
  if (line == lastLine) {
    // no, get out
    return line;
  }
